...
import { updateCollectionPointStatus, updateCollectionPointVerification } from "@/actions/points";
  ...
  return (
    <div className="flex items-center gap-2">
      <InlineSelect
        ...
        onChange={(v) => updateCollectionPointVerification(id, v as VerificationLevel)}
      />
      <InlineSelect
        value={status}
        options={Object.entries(pointStatusLabels).map(([value, label]) => ({ value, label }))}
        onChange={(v) => updateCollectionPointStatus(id, v as PointStatus)}
      />
    </div>
  );
}
