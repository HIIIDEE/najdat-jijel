...
export const transportOfferSchema = z.object({
  driver_name: z.string().trim().min(2, "الاسم مطلوب"),
  phone: z
    .string()
    .trim()
    .regex(/^0[5-7][0-9]{8}$/, "رقم هاتف جزائري غير صحيح (مثال: 0555xxxxxx)"),
  origin_wilaya: z.string().trim().min(2, "نقطة الانطلاق مطلوبة"),
  origin_note: z.string().trim().max(300).optional().or(z.literal("")),
  destination_wilaya: z.string().trim().min(2),
  destination_note: z.string().trim().max(300).optional().or(z.literal("")),
  vehicle_type: z.enum(["car", "van", "small_truck", "medium_truck", "large_truck", "trailer"]),
  max_capacity_kg: z.number().positive().optional(),
  available_space_note: z.string().trim().max(300).optional().or(z.literal("")),
  travel_date: z.string().optional().or(z.literal("")),
  time_window: z.string().trim().max(100).optional().or(z.literal("")),
